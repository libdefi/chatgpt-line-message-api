...
async function isHotswappableLambdaFunctionChange(logicalId, change, evaluateCfnTemplate) {
    ...
    const ret = [];
    const classifiedChanges = common_1.classifyChanges(change, ['Code', 'Environment', 'Description']);
...
function classifyAliasChanges(change) {
    const ret = [];
    const classifiedChanges = common_1.classifyChanges(change, ['FunctionVersion']);
    classifiedChanges.reportNonHotswappablePropertyChanges(ret);
    const namesOfHotswappableChanges = Object.keys(classifiedChanges.hotswappableProps);
    if (namesOfHotswappableChanges.length > 0) {
        ret.push({
